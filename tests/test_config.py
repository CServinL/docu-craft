...
from docify.config import load_settings, _HARDCODED_DEFAULTS


class TestLoadSettings:
    def test_hardcoded_defaults_with_no_configs(self, tmp_dir, monkeypatch):
        import docify.config as cfg
    ...
    def test_project_config_overrides_user_config(self, tmp_dir, monkeypatch):
        ...
        project_dir.mkdir()
        (project_dir / ".docify.yaml").write_text("defaults:\n  engine: reportlab\n")
    ...
    def test_docify_yaml_takes_priority_over_dotdocify_yaml(self, tmp_dir, monkeypatch):
        import docify.config as cfg
        monkeypatch.setattr(cfg, "USER_CONFIG_FILE", tmp_dir / "no.yaml")
        (tmp_dir / ".docify.yaml").write_text("defaults:\n  theme: handout\n")
        (tmp_dir / "docify.yaml").write_text("defaults:\n  theme: scholar\n")
        result = load_settings(tmp_dir)
        # .docify.yaml is checked first
        assert result["theme"] == "handout"
