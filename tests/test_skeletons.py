...
class TestSkeletonManager:
    ...
    def test_user_yaml_loaded(self, tmp_dir, monkeypatch):
        user_skeletons = tmp_dir / "skeletons"
        user_skeletons.mkdir()
        (user_skeletons / "mythesis.yaml").write_text(
            "sections:\n  - heading: Intro\n    required: true\n"
        )
        import docify.skeletons.manager as mgr
        monkeypatch.setattr(mgr, "USER_SKELETONS_DIR", user_skeletons)
