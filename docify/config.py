...
_BUNDLED_ROOT = Path(__file__).parent / "assets"   # docify/assets/ — mirrors ~/docify/
...
def load_settings(project_dir: Path | None = None) -> dict:
    ...
    # 2. Project-level: .docify.yaml (walks up from document dir)
    if project_dir:
        for candidate in [project_dir, *project_dir.parents]:
            for fname in (".docify.yaml", "docify.yaml"):
